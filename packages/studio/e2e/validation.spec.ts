...
test("validation panel lists quality issues in the tree view", async ({ page }) => {
  ...
  await expect(page.getByTestId("validation-issue-IMAGE_MISSING").first()).toBeVisible();
  await expect(page.getByTestId("validation-issue-VARIABLE_UNWRITTEN").first()).toBeVisible();
});
